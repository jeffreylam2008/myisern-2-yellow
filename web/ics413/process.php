...
if(isset($_POST['coll_sql_export']))
{
	...
	for($i = 0; $i < $numOfRows; $i++)
	{
		...
		$row['c_id'],$row['c_name'],$row['c_organizations'],$row['c_types'],$row['c_years'],$row['c_outcome_types'],$row['c_description']);
		if($i == ($numOfRows-1))
			fprintf($file, ";");
		else
			fprintf($file, ",\n");
...
if(isset($_POST['sql_import']))
{
	...
	else
	{
		...
		fclose($file);
		$message = "<tr><td bordercolor='#000099' bgcolor='#0066FF' align='center'><h4><font color='#FFFFFF'>researcher.sql imported!</font></h4><br>";
		$message .= "<input type='button' value='CLOSE' name='close' onclick='javascript:window.close()'></td></tr>";
	}
}
?>		
...
function readSpecialChar($sting, $sChar)
{
	for($i = 0; $i < sizeof($string); $i++)
	{
		if($string[$i] == $sChar)
			printf("find special char"); 
	}
